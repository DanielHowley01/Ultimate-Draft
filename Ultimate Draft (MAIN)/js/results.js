...
async function loadData(lobbyId) {
  ...
  allPicks = (picks || []).map(p => ({
    ...p,
    username: lobbyPlayers.find(lp => lp.user_id === p.user_id)?.username || 'Player'
  }));
}
...
async function settleResults() {
  ...
  // Mark lobby settled
  await sb.from('lobbies').update({ settled: true, status: 'complete' }).eq('id', currentLobby.id);
...
async function renderBreakdown() {
  ...
  document.getElementById('results-breakdown').innerHTML = myPicks.map(pick => {
    ...
    const eventRows = playerEvents.map(e => {
      ...
      const label = e.type === 'goal' ? `Goal (${e.minute}')` : e.type === 'assist' ? `Assist (${e.minute}')` : e.type === 'yellow_card' ? `Yellow card (${e.minute}')` : `Red card (${e.minute}')`;
      return `
