...
async function loadFixtures() {
  ...
  // Filter out finished fixtures
  allFixtures = fixtures.filter(f => f.status !== 'ft');
...
async function loadMyGames() {
  ...
  const { data: mySlots, error } = await sb
    ...
    .select(`
      lobby_id,
      lobbies (
        id, status, entry_fee, current_pick, draft_order,
        fixture_id,
...
function renderMyGames(lobbies) {
  ...
  Object.values(grouped).forEach(({ fixture: f, lobbies: fLobbies }) => {
    ...
    fLobbies.forEach((lobby, i) => {
      ...
      const statusLabel = lobby.status === 'waiting'  ? 'Waiting room'
                        : lobby.status === 'drafting' ? 'Drafting...'
                        : lobby.status === 'active'   ? 'Live match'
                        : 'Finished';

      const statusCls = lobby.status === 'active'   ? 'game-status-live'
                      : lobby.status === 'drafting' ? 'game-status-draft'
                      : 'game-status-wait';

      const dest = lobby.status === 'waiting' || lobby.status === 'drafting'
