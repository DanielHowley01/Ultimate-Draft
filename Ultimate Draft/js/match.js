...
async function loadLobbyData(lobbyId) {
  const { data: lobby, error } = await sb
    .from('lobbies')
    .select(`*, fixtures(*), lobby_players(user_id, username)`)
    .eq('id', lobbyId)
    .single();
...
async function loadPicksAndEvents() {
  ...
  // Cache for getPlayerEvents() — this was read elsewhere but never set,
  // so player cards never showed their goal/card badges
...
function renderMyTeam() {
  ...
  const sortedNames = [...myPickNames].sort((a, b) => {
    const ra = POS_ORDER[String(getPlayerPos(a)).toUpperCase()] ?? 2;
    const rb = POS_ORDER[String(getPlayerPos(b)).toUpperCase()] ?? 2;
  ...
  container.innerHTML = sortedNames.map(name => {
    ...
    const eventBadges = events.map(e => {
      const t = eventKind(e);
      const cls  = t === 'goal' ? 'goal' : t === 'assist' ? 'assist' : 'card';
      const icon = t === 'goal' ? '⚽' : t === 'assist' ? '🅰' : t === 'yellow_card' ? '🟨' : '🟥';
...
function renderEvents(events) {
  ...
  feed.innerHTML = events.map(e => {
    ...
    const t     = eventKind(e);
    const icon  = t === 'goal' ? '⚽' : t === 'assist' ? '🅰' : t === 'yellow_card' ? '🟨' : t === 'red_card' ? '🟥' : '📋';
    ...
    return `
      <div class="match-event-item ${isMyPlayer ? 'my-event' : ''}">
        ...
        <div class="event-info">
          ...
          <span class="event-team">${escHtml(e.team || e.team_name || '')}</span>
        </div>
...
// Normalise a match_events row to goal / assist / yellow_card / red_card.
// The sync function stores event_type ('goal', 'card', 'assist') + detail
// ('Yellow Card' / 'Red Card'), so cards need the detail to disambiguate.
