...
function watchAd() {
  ...
  // Check if AdSense rewarded is available
  if (typeof window.adsbygoogle === 'undefined') {
    // AdSense not loaded yet (dev/adblocker) — simulate for testing
    simulateAd();
    return;
  }
  ...
  try {
    ...
    // Load rewarded ad
    // Replace ca-pub-XXXXXXXXXXXXXXXX/XXXXXXXXXX with your actual ad unit ID
    const adBreak = window.adBreak || function() {};
    adBreak({
      type: 'reward',
      name: 'coins_reward',
      beforeAd: () => {
      ...
      adDismissed: () => {
        ...
        btn.disabled = false;
        btn.textContent = '▶ Watch ad · +50';
        toast('Watch the full ad to earn coins', 'inf');
      },
      adViewed: async () => {
        // Ad completed — award coins
        await awardCoins(50);
      },
      afterAd: () => {
        // Reset button with cooldown
        startAdCooldown(btn);
      }
    });
  } catch (e) {
...
// Simulate ad — disabled until AdSense approved
async function simulateAd() {
  const btn = document.getElementById('watch-ad-btn');
  btn.disabled = false;
  btn.textContent = '▶ Watch ad · +50';
  toast('Ads coming soon!', 'inf');
}

async function awardCoins(amount) {
  const newCoins = currentProfile.coins + amount;
  const { error } = await sb.from('profiles')
    .update({ coins: newCoins })
    .eq('id', currentUser.id);

  if (error) {
    toast('Failed to add coins', 'err');
    return;
  }
...
function startAdCooldown(btn) {
  ...
  const interval = setInterval(() => {
    ...
    btn.textContent = `Next ad in ${mins}:${String(secs).padStart(2, '0')}`;
    btn.disabled = true;
